...
export const ShowItems = ({ productData2 = [], isLoading = true }) => {
    const { categoria } = useParams();

    return (
        <div className="flex gap-5 justify-center flex-wrap mt-3 ">
            {isLoading ? (
                <div className="p-8 drop-shadow-sm w-full h-full max-w-sm bg-white rounded-lg relative text-gray-900">
                    <Skeleton>
                        <div className="flex gap-4 flex-col">
                            <div className="flex justify-center items-center w-80 h-80 "></div>
                            <div className="flex justify-center items-center w-80 h-5 "></div>
                        </div>
                    </Skeleton>
                </div>
            ) : (
